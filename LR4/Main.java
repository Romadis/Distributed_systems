...
public class Main {
    ...
    // 2 - Broadcast/Reduce
    // 3 - Scatter/Gather
    private static final int EXCHANGE_TYPE = 2;
    ...
    private static void computeBroadcastReduce(int vectorSize, int rank, int size) throws Exception {
        System.out.println("Процесс №" + rank + " начал работу");

        int baseChunkSize = vectorSize / size;
        int remainder = vectorSize % size;

        if (rank == 0) {
            System.out.println("Главарь генерирует векторы");
            ...
            System.out.println("Главарь начинает параллельное вычисление");
            long parStartTime = System.nanoTime();
            ...
            for (int i = 1; i < size; i++) {
                ...
                System.out.println("Главарь отправляет данные процессу " + i);
            ...
            System.out.println("Главарь собирает результаты");
            ...
            System.out.println("Процесс №" + rank + " ждёт данные");
        ...
        System.out.println("Процесс №" + rank + " завершил работу");
    }
